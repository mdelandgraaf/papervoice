...
3. **`scripts/healthcheck`** exercises every vendor API surface we depend on against live APIs: ElevenLabs TTS synth roundtrip, ElevenLabs STT (Scribe) roundtrip, LiveKit room create/delete, a real LiveKit room **join** over WebRTC (not just the server registry), and LiveKit SIP trunk-list reachability (0 trunks is healthy — telephony isn't provisioned yet, this only catches the SIP API itself breaking). It also validates both the **static** persona voice roster and the **live** Paperclip-configured roster (`metadata.papervoice.voice_id`) for **streamability on the current tier** — the voices real calls actually use. Run before every board meeting and after any update.
   - **Non-streamable voices are caught, not fatal (PER-306 → PER-311).** A configured `voice_id` that the realtime streaming WebSocket won't accept makes it reject *every* synthesis with `voice_id_does_not_exist`, leaving that agent silent for the whole call with no recovery — a 1:1 direct call to that agent is then completely dead. There are **two** ways a voice fails to stream: (a) it isn't in the account library at all (PER-306); or (b) it *is* in the library and even synthesizes over REST, but it's a **professional/cloned** voice this tier isn't entitled to stream (PER-311 — the actual live outage: `P4Dhdy…` "Andy C", category `professional`, on a `payg` account with `can_use_professional_voice_cloning:false`). So the signal is **streamability = category × subscription entitlement**, not library presence and *not* a REST roundtrip (the lenient REST `/text-to-speech` path synths the exact voice that silences live calls). Premade voices stream on any tier; professional/cloned only with the matching entitlement (`/v1/user/subscription`). Two defenses: (a) the healthcheck's live-roster check (`streamability_report`) flags any configured voice that won't stream before a call; (b) at call time `elevenlabs.resolve_voice_id()` (used by `plugin_tts`) gates on `_streamable_voice_ids()` and falls back to a streamable premade when the configured one isn't streamable, so a wrong-but-audible voice is served instead of a dead line. `DEFAULT_VOICE_ID` and the fallback candidates are premade voices present on the account (Sarah/George), so the fallback itself always streams.
